                        ...
                        if (snapshot.connectionState != ConnectionState.done) {
                          ...
                          if (data != null) {
                            return data.data.length > 1
                                ? DropdownButton<Data>(
                                    //value: dropdownValue,
                                    value: dropdownValue == null ? dropdownValue : snapshot.data.data.data.where((i) => i.name == dropdownValue.name).first,
                                    underline: SizedBox(),
                                    isExpanded: true,
                                    icon: const Icon(
                                    ...
                                    hint: Text(
                                      ...
                                      style:
                                          TextStyle(fontFamily: proxima_nova_reg),
                                    ),
                                    style:
                                        const TextStyle(color: Palette.loginhead),
                                    onChanged: (Data newValue) {
                                      setState(() {
                                        ...
                                        selectedItemId = snapshot
                                            ...
                                            .id;
                                        print(
                                            'value ${newValue.name} $selectedItemId');
                                      });
                                    },
                                    items:
                                        snapshot.data.data.data.map((Data item) {
      ...
      bottomNavigationBar: MaterialButton(
        ...
        onPressed: () {
          if(selectedItemId != null){
            Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => AddNewItem(dropdownValue, false, null)),);

          }else{
            CommonFunction.toastMessage('Please Select Food Type');
          }
        },
              ...
              onPressed: () {
                if (_textMenuNameController.text.isEmpty) {
                  setState(() {
                    ...
                    CommonFunction.toastMessage(getTranslated(context, empty_error_text));
                  });
                } else {
                  ...
                  FutureBuilder<BaseModel<CommonResponse>>(
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.connectionState != ConnectionState.done) {
                        ...
                        if (data != null) {
                          Navigator.pop(context);
                          return CommonFunction.toastMessage(data.data);
                        } else {
                          return  CommonFunction.toastMessage(snapshot.error);
                        }
                      }
                    },
                  );
